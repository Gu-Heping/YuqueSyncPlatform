...
const ChatWidget = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed inset-0 z-50 md:inset-auto md:bottom-6 md:right-6 md:w-[450px] md:h-[600px] flex flex-col bg-white md:rounded-2xl shadow-2xl md:border border-gray-200 overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
          {/* Header */}
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-6">
            {messages.map((msg, idx) => (
              ...
              >
                ...
                <div className={`relative max-w-[85%] md:max-w-[80%] rounded-2xl px-4 py-3 text-sm shadow-sm overflow-hidden ${
                  ...
                  {msg.role === 'assistant' ? (
                    <div className="prose prose-sm max-w-none prose-p:my-1 prose-pre:my-2 prose-pre:bg-gray-800 prose-pre:text-gray-100 break-words">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm]}
                        components={{
                          pre({children}) {
                            return <div className="not-prose overflow-x-auto my-2 rounded-lg bg-gray-800 p-3 text-gray-100">{children}</div>
                          },
            ...
            {loading && (
              <div className="flex gap-3">
                <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 shadow-sm">
