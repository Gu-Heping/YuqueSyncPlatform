...
### Q: 我能否一个不用端口(http://ip/)，一个用端口(http://ip:80)？
**答：不可以。**
在 HTTP 协议中，`http://139.224.72.130/` 默认就是访问 **80** 端口。所以 `http://ip/` 和 `http://ip:80` 是完全同一个地址。
你无法让两个不同的容器同时占用同一个 IP 的 80 端口。

**正确做法**：
1.  **实例 A (主)**：占用宿主机 **80** 端口。
    *   访问地址：`http://139.224.72.130` (无需输入端口)
    *   配置：`HOST_PORT=80`
2.  **实例 B (副)**：占用宿主机 **81** 端口 (或其他空闲端口)。
    *   访问地址：`http://139.224.72.130:81`
    *   配置：`HOST_PORT=81`
